<template>
  <div class=" relative" v-show="slides.length > 0">
    <client-only>
      ...
      <span
        :id="`swiper-${uniqueId}-button-next`"
        class="swiper-button-prev flex items-center justify-center"
      >
        <img src="/icons/gallery-arrow-right.svg" alt="Next" />
      </span>
      <span
        :id="`swiper-${uniqueId}-button-prev`"
        class="swiper-button-next flex items-center justify-center"
      >
...
export default {
  ...
  data() {
    return {
      ...
      uniqueId: uuidv4(), // Unique identifier for navigation buttons
    };
  },
};
</script>
